...
export default function Catering() {
  ...
  const searchCatering = async () => {
    ...
    setLoading(true);
    try {
      const API_KEY = process.env.NEXT_PUBLIC_GOOGLE_API_KEY; // Using your existing key
      const CX = process.env.NEXT_PUBLIC_GOOGLE_CX;           // Using your existing CX
      
      // We search for catering in the specific city entered by the user
    ...
    <main className="min-h-screen bg-gradient-to-b from-white to-orange-50 px-6 py-12 flex flex-col items-center">
      ...
      <div className="w-full max-w-md flex gap-2 mb-12">
        ...
        <button 
          ...
          className="bg-orange-500 text-white px-6 py-3 rounded-full font-bold shadow-lg hover:bg-orange-600 transition"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-6xl text-left">
        {results.length > 0 ? (
          results.map((item, index) => (
            <div key={index} className="p-6 bg-white rounded-2xl shadow-md border border-orange-100 hover:shadow-xl transition flex flex-col justify-between">
              ...
              <a 
                ...
                className="text-orange-600 font-bold text-sm hover:underline mt-2 flex items-center gap-1"
              >
                View Details →
              </a>
            </div>
          ...
          !loading && city && <p className="text-gray-400 text-center col-span-full italic">Search for caterers in your location...</p>
        )}
      </div>
    </main>
  );
